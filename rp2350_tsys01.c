...
int tsys01_read(long * temp_thousandths_p) {
    ...
    do {
        if (-1 == i2c_write_blocking(i2c0, 0x77, &(uint8_t){ 0x48 }, 1, false)) break;
        ...
        if (-1 == i2c_write_blocking(i2c0, 0x77, &(uint8_t){ 0x00 }, 1, false) ||
            -1 == i2c_read_blocking(i2c0, 0x77, bytes, 3, false)) break;

        i2c_release();

        const uint32_t adc24 = (bytes[0] << 16) | (bytes[1] << 8) | bytes[2];

        /* TODO: this is literal from the datasheet, if possible rework all this sketchy
         math to return temperature in thousandths of a deg without doing floating point */
